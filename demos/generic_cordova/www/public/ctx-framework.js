...
module.exports = {
    parallel : function(tasks /* tasks Object | Array [, options Object, callback Function]*/){
        tasks     =    typeof tasks == 'object' ? tasks : [];
        var options  =    typeof arguments[1] == 'object' ? arguments[1] : {};
        var done =    typeof arguments[1] == 'function' ? arguments[1] :
                    typeof arguments[2] == 'function' ? arguments[2] : function(){};

        var n_tasks  = Object.keys(tasks).length; // Number of tasks left
        var task_data  = tasks instanceof Array ? [] : {}; // Task Data, data given to task callback.
        var complete   = false; // Have tasks complete?
        var errors = tasks instanceof Array ? [] : {}; // Store errors if the silent option is passed.
        ...
        if(n_tasks === 0){
            ...
            for(var id in tasks){
                ...
                task.do(function(error, data){
                    n_tasks--;

                    if(!complete){
                        task_data[this.id] = data;

                        if(error){
                            errors[this.id] = error;

                            if(options.silent !== true){
                                complete = true;

                                var error_keys = Object.keys(errors); // Error keys
                                finalize(error_keys.length === 0 ? null : error_keys.length === 1 ? errors[error_keys[0]] : errors, task_data);
                                return;
                            }
                        }
    ...
    series : function(tasks /* tasks Object | Array [, options Object, callback Function]*/){
        tasks     =    typeof tasks == 'object' ? tasks : [];
        var options  =    typeof arguments[1] == 'object' ? arguments[1] : {};
        var done =    typeof arguments[1] == 'function' ? arguments[1] :
                    typeof arguments[2] == 'function' ? arguments[2] : function(){};
        ...
        if(ids.length === 0){
            ...
            // I am reversing the task_ids so I can pop them off.
            ids = ids.reverse();
            next();
...
Task.prototype.do = function(done){
    var complete = false; // Has task complete?
    var self = this;
    try{
        this.task(function(error, data /* Data */){
            if(complete){
                console.error('Async task "'+self.id+'" callback already called.');
            }else{
                complete = true;
                done.call(self, error, data);
            }
        });
    }catch(error){
        if(complete){
            console.error('Async task "'+self.id+'" caught an error after task had executed callback.');
            console.error(error);
        }else{
            complete  = true;
...
},{"e/U+97":35}],5:[function(require,module,exports){
...
},{"./debug":14,"e/U+97":35}],14:[function(require,module,exports){
...
},{"../transport":17,"component-inherit":12,"debug":13,"engine.io-parser":24,"parseqs":33,"xmlhttprequest-ssl":23,"yeast":46}],22:[function(require,module,exports){
(function (global){
/**
 * Module dependencies.
 */
...
},{"../transport":17,"component-inherit":12,"debug":13,"engine.io-parser":24,"parseqs":33,"ws":8,"yeast":46}],23:[function(require,module,exports){
(function (global){
...
},{"./keys":25,"./utf8":26,"after":1,"arraybuffer.slice":2,"base64-arraybuffer":6,"blob":7,"has-binary2":27}],25:[function(require,module,exports){
...
},{"e/U+97":35}],33:[function(require,module,exports){
/**
...
},{"./on":38,"./socket":39,"backo2":5,"component-bind":10,"component-emitter":11,"debug":13,"engine.io-client":15,"indexof":30,"socket.io-parser":42}],38:[function(require,module,exports){
...
},{"./binary":41,"./is-buffer":43,"component-emitter":11,"debug":13,"has-binary2":27}],43:[function(require,module,exports){
(function (global){
...
},{"../common/setups/interface.js":50,"e/U+97":35,"next-tick":32}],48:[function(require,module,exports){
(function (process){
...
},{"async-min":3,"e/U+97":35,"module":9}],49:[function(require,module,exports){
(function (process,global){
...
function Service(_package){ // _package contains all relevant assets for the service.
	...
	Object.defineProperty(this, 'path', {
		value: typeof _package.path == 'string' ? _package.path : "" // The path to the service from CTX.root, ensure path has unix separators and is relative
	});

	Object.defineProperty(this, '_origin', {
		value: typeof _package._origin == 'string' && _package._origin.length ? _package._origin : CTX.config.origin
	});
	...
	this._id = _package._id;

	Object.defineProperty(this, '_protocol', {
		get: function(){
			return '__CTX__SERVICE_'+_id;
		}
	});
		...
		set : function(new_socket){
			...
			switch (new_socket.constructor.name){
				...
				default:
					...
					if(!socket) socket = CTX._origins[self._origin] || new io.Socket({});
					break;
			}
...
Service.prototype.stop = function(transmit){
	if(!this._flags.stopped){
		...
		this.off();
		this._status = -1;
		delete Service.running[this._id];
	}
}
...
},{"async-min":3,"component-emitter":11,"e/U+97":35,"socket.io-client":36,"to-array":45}],50:[function(require,module,exports){
(function (process,global){
var io = require('socket.io-client');
var Emitter = require('component-emitter');
var toArray = require('to-array');
...
CTX.start = function start(){
    ...
    options.origin = parseuri(typeof options.origin == 'string' ? options.origin : CTX.config.origin).source;

    var socket = CTX._origins[options.origin];
    if(!socket){
        socket = io(options.origin+'/__CTX__');
        CTX._origins[options.origin] = socket;
    }

    // SETUP START NEW!!!!

    // Sockets are reused for the base origin transfer layer.
    // Services will create namespaces for each of their _id's.
    var events = new Emitter();

    process.nextTick(function(){
        socket.emit('__CTX__START', options, function(error, service){
...
},{"../classes/Module.js":48,"../classes/Service.js":49,"./localhost.js":51,"component-emitter":11,"e/U+97":35,"parseuri":34,"socket.io-client":36,"to-array":45}],51:[function(require,module,exports){
// GLOBAL PROTOCOLS
...
var manager = new io.Manager('', {autoConnect:false});
var localhost = manager.socket('/__CTX__');

// This causes the localhost to emit on itself rather than trying to
// communicate over a socket.
localhost.emit = (new Emitter()).emit.bind(localhost);

var origin = '';
Object.defineProperty(CTX.config, 'origin', {
    set : function(new_origin){
        if(typeof new_origin != 'string') return;

        CTX._origins[new_origin] = CTX._origins[origin];
        delete CTX._origins[origin];
...
Object.defineProperty(CTX, '_localhost', {
    set : function(socket){
        CTX._origins[origin] = socket;
    },
    get : function(){
        return CTX._origins[origin];
    }
});

CTX._localhost = localhost;
...
localhost.on('__CTX__START', function(options, done){
    if(options.unique !== true){
        var id = CTX._generateId(options.path);
        if(CTX.Service.running[id]){
            done(undefined, CTX.Service.running[id]);
            return;
        }
    }

    fetchService(options, function(err, _package){
        if(err){
            done(err);
        }else{
            // Because localhost is not a socket the service will
            // replace the provided socket with a blank placeholder socket.
            // This is the desired behaviour as there is no foreground
            // connected yet, so we want a disconnected socket so that the
            // events will buffer until an active socket is connected.

            var id = options.unique === true ?  CTX._generateId() : CTX._generateId(options.path);

            try{
                var service = new CTX.Service(_package, options);
                service._id = id;
    ...
    function fetchService(options, done){
        var _package = CTX.Service.cache[options.path];
            ...
            CTX._fs.readFile(base_path+'/service.ctx.json', function(error, file){
                ...
                async.parallel({
                    ...
                    if(error){
                        ...
                        CTX.Service.cache[_package.path] = _package;
...
localhost.connect = function(socket){
    ...
    socket.on('__CTX__CONNECT', function(_id){
        try{
            CTX.Service.running[_id]._socket = socket;
        }catch(error){}
    });
    ...
    socket.on('__CTX__START', function(options, done){
        ...
        options.new = true; // Force new start from clients.

        localhost.emit('__CTX__START', options, function(err, _package){
            if(err){
                done(err);
            }else{
                try{
                    var service = new CTX.Service(_package);
                }catch(err){
                    done(err);
                    return;
                }
...
},{"async":4,"component-emitter":11,"socket.io-client":36,"to-array":45}]},{},[47])