...
CTX.start = function start(){
    ...
    process.nextTick(function(){
        if(options.unique !== true){
            ...
            var id = CTX._generateId(options.name || options.path);
...
localhost.on('__CTX__START', function(options, done){
    fetchService(options, function(err, _package){
        if(err){
            ...
            if(!_package._id){
                // If unique is not requested than generate id based on name or path
                var id = options.unique === true ?  CTX._generateId() : CTX._generateId(options.name || options.path);
            }

            try{
                var service = new CTX.Service(_package);
                service.id = id; // If id is already set than this will do nothing
...
localhost.connect = function(socket){
    ...
    socket.on('__CTX__START', function(options, done){
        ...
        //options.unique = true; // Force new start from clients.
